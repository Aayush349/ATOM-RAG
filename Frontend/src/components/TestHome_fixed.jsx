...
const handleSend = async () => {
  ...
  return (
    <div className="flex flex-col items-center min-h-[100dvh] overflow-hidden bg-gradient-to-tr from-[#141E30] via-[#243B55] to-[#141E30] text-white p-4 relative overflow-hidden">
    ...
    : "bg-[#232526]/80 text-purple-100 border-purple-400"
          ...
          >
            <motion.input
              ...
              placeholder= {isLoading ?"Ask ATOM something..."}
...
>
  <motion.input
    type="text"
    value={input}
    onChange={(e) => setInput(e.target.value)} // ✅ Only one onChange
    onKeyDown={(e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        handleSend();
    ...
    className="flex-1 p-2 rounded-xl border border-slate-600 text-white bg-gray-900 shadow focus:outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-200"
    placeholder={isLoading ? "Ask ATOM something..." : "Type your question..."}
    whileFocus={{ scale: 1.03, borderColor: "#a78bfa" }}
    disabled={isLoading}
  />
  ...
  <motion.button
    onClick={handleSend}
    whileTap={{ scale: 0.9 }}
    className="bg-purple-600 hover:bg-purple-700 text-white p-2 rounded-full shadow transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-purple-400"
  >
    <IoSendSharp size={20} />
  </motion.button>
