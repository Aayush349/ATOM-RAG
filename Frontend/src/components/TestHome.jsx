...
const handleSend = async () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    setError("");
    setChatOpen(true); // Open chat when file is uploaded
...
{file && (
  <div className="flex items-center justify-between w-full max-w-5xl mb-4">
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="text-lg md:text-xl font-semibold text-purple-200 bg-[#232526]/70 px-4 py-2 rounded-xl shadow mr-3"
    >
      {file.name}
    </motion.div>
